...
df = companies.merge(
    sectors, left_on="id", right_on="company_id", how="left", suffixes=("", "_sec")
)

df["bse_link"] = df["id"].apply(
    lambda x: f"https://www.bseindia.com/corporates/List_Scrip.html?scripcode={x}"
)
df["nse_link"] = df["id"].apply(
    lambda x: f"https://www.nseindia.com/get-quotes/equity?symbol={x}"
)

cols = ["id", "company_name", "broad_sector", "website", "bse_link", "nse_link"]
display_df = df[cols].rename(
    columns={
        "id": "Ticker",
